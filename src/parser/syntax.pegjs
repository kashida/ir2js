...
DoubleStringCharacters
  = c:DoubleStringCharacter+ { return c.join(''); }

SingleStringCharacters
  = c:SingleStringCharacter+ { return c.join(''); }
...
NonEscapeCharacter
  = (!EscapeCharacter / LineTerminator) c:SourceCharacter { return c; }
...
RegularExpressionBody
  = c:RegularExpressionChar* { return c.join(''); }
...
RegularExpressionClass
  = '[' c:RegularExpressionClassChars ']' { return '[' + c + ']'; }

RegularExpressionClassChars
  = c:RegularExpressionClassChar* { return c.join(''); }
...
RegularExpressionFlags
  = p:IdentifierPart* { return p.join(''); }
...
IdentifierName 'identifier'
  = s:IdentifierStart p:IdentifierPart* {
      return s + p.join('');
    }
...
StringLiteral 'string'
  = p:('"' DoubleStringCharacters? '"' / "'" SingleStringCharacters? "'") {
      return p.join('');
    }
...
ArrayBlockMarker = '[' _ '#' _ ']' { return M('a'); }
ObjectBlockMarker = '{' _ '#' _ '}' { return M('o'); }
ParameterBlockMarker = '(' _ '#' _ ')' { return M('p'); }
...
MemberExpression
  = (
        PrimaryExpression
      / 'new' __ constructor:MemberExpression w1:_ args:Arguments
    )
    (
        _ '[' _ Expression _ ']'
      / _ '.' _ IdentifierName
...
TypeExpression
  = c:(!'\\' SourceCharacter)* {
      return c.map(function(c) { return c[1]; }).join('');
    }
...
Expression
  = head:AssignmentExpression tail:(_ ',' __ AssignmentExpression)*
...
VariableDeclarationList
  = head:VariableDeclaration tail:(_ ',' __ VariableDeclaration)*
