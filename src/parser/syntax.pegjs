...
IdentifierName 'identifier'
  = start:IdentifierStart parts:IdentifierPart* {
      return T(start + parts.join(''));
    }

Identifier 'identifier'
  = !ReservedWord name:IdentifierName { return name; }

NullLiteral = 'null' { return T('null'); }
...
ObjectBlockMarker = '{' w0:_ '#' w1:_ '}' { return C(w0, M('o'), w1); }

ParameterBlockMarker = '(' w0:_ '#' w1:_ ')' { return C(w0, M('p'), w1); }

FunctionBlockMarker = '##' { return M('f'); }
...
PrimaryExpression
  ...
  / ArrayLiteral
  / ObjectLiteral
  / '(' w0:_ expr:Expression w1:_ ')' { return C('(', w0, expr, w1, ')'); }

ArrayLiteral
  = '[' w0:_ elements:ElementList? w1:_ ']' {
      return elements ? C('[', w0, elements, w1, ']') : T('[]');
    }

ElementList
  = head:AssignmentExpression
    tail:(_ ',' _ AssignmentExpression)* {
      var ret = T(head);
      tail.forEach(function(t) {
        ret.add(t[0], ', ', t[2], t[3]);
      });
...
ObjectLiteral
  = '{' w:_ properties:(PropertyNameAndValueList _ )? '}' {
      return properties ? C('{', w, properties, '}') : T('{}');
    }
...
PropertyAssignment
  = name:PropertyName w0:_ ':' w1:_ value:AssignmentExpression {
      return C(name, w0, ': ', w1, value);
    }
...
MemberExpression
  = base:(
        PrimaryExpression
      / 'new' w0:_ constructor:MemberExpression w1:_ args:Arguments {
          return C('new ', w0, constructor, w1, args);
        }
    )
    accessors:(
        _ '[' _ Expression _ ']'
      / _ '.' _ IdentifierName
    )* { return C(base, accessors); }

NewExpression
  = MemberExpression
  / 'new' w:_ constructor:NewExpression { return C('new ', w, constructor); }
...
TypeLiteral
  = '\\' type:TypeExpression '\\' {
      return TT(type.toString(), C('\\', type, '\\'));
    }

TypeExpression
  = chars:(!'\\' SourceCharacter)* {
      return T(chars.map(function(c) { return c[1]; }).join(''));
    }
...
ConditionalExpression
  = cond:LogicalORExpression w0:_
    '?' w1:_ trueExpr:AssignmentExpression w2:_
    ':' w3:_ falseExpr:AssignmentExpression {
      return C(cond, w0, ' ? ', w1, trueExpr, w2, ' : ', w3, falseExpr);
    }
  / LogicalORExpression

DeclareAssignmentExpression
  = left:Identifier w0:_ ':=' w1:_ right:AssignmentExpression {
      return C(left, w0, ' = ', w1, right).prepend(C('var ', left));
    }

AssignmentExpression
  = left:LeftHandSideExpression w0:_
    op:AssignmentOperator w1:_
    right:AssignmentExpression { return C(left, w0, ' ', op, ' ', w1, right); }
  / DeclareAssignmentExpression
  / ConditionalExpression

Expression
  = head:AssignmentExpression
    tail:(_ ',' _ AssignmentExpression)* { return opList(head, tail, true); }
